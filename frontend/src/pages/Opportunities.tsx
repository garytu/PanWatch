...
const displayActionLabel = (item: StrategySignalItem) => {
  const action = (item.action || '').toLowerCase()
  if (!item.is_holding_snapshot && action === 'hold') return '觀望'
  if (!item.is_holding_snapshot && action === 'add') return '建倉'
...
const toSignalFromCandidate = (row: EntryCandidateItem): StrategySignalItem => {
  ...
  const sourceLabel = row.candidate_source_label || (source === 'market_scan' ? '市場池' : source === 'mixed' ? '市場+關注' : '關注池')
  const riskLevel: 'low' | 'medium' | 'high' = Number(row.score || 0) >= 85 ? 'high' : Number(row.score || 0) >= 70 ? 'medium' : 'low'
  const riskLabel = riskLevel === 'high' ? '高風險' : riskLevel === 'low' ? '低風險' : '中風險'
  return {
    id: Number(row.id || 0),
    ...
    strategy_name: (row.strategy_labels && row.strategy_labels[0]) || '候選建議',
    strategy_version: 'v1',
    ...
    action_label: row.action_label || '觀望',
    signal: row.signal || '',
    reason: row.reason || '',
...
const formatEntryDisplay = (action: string | undefined, entryLow: number | null, entryHigh: number | null) => {
  ...
  const key = (action || '').toLowerCase()
  if (key === 'buy' || key === 'add') return '待補充入場位'
  return '當前不建議開倉'
}
...
export default function OpportunitiesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        if (!msg.includes('超時')) throw firstErr
        try {
          ...
          if (!secondMsg.includes('超時')) throw secondErr
      ...
      if ((!data.items || data.items.length === 0) && market !== 'ALL') {
        ...
        if (fallback.items && fallback.items.length > 0) {
          setError(`當前${marketLabel(market)}暫無滿足條件機會，已展示全市場結果`)
          data = fallback
        }
      }
      setItems(data.items || [])
  ...
  const pollRefreshCompletion = useCallback(async () => {
    ...
    for (let i = 0; i < maxPolls; i += 1) {
      try {
        ...
        if (!state.running) {
          if (state.last_error) {
            setError(`後臺重新整理失敗: ${state.last_error}`)
          } else {
            setError('')
          }
    ...
    await Promise.all([load(), loadStats()])
    setError((prev) => prev || '重新整理任務仍在後臺執行，請稍後重試')
  }, [load, loadStats])
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      if (resp.queued) {
        setError(resp.accepted ? '已提交後臺重新整理任務，完成後自動更新' : '重新整理任務已在執行中，完成後自動更新')
        void pollRefreshCompletion()
        return
      }
      await Promise.all([load(), loadStats()])
    } catch (e) {
      const msg = e instanceof Error ? e.message : '重新整理失敗'
      if (msg.includes('超時')) {
        setError('重新整理任務耗時較長，已在後臺繼續執行，請稍後再點重新整理')
        await load()
      } else {
        setError(msg)
      }
  ...
  const regimeSummary = useMemo(() => {
    return (stats?.regimes || []).map((r) => ({
      market: r.market,
      label: r.regime_label || r.regime || '震盪',
    ...
    <div className="page-container pb-10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        <div className="card p-3">
          <div className="text-[11px] text-muted-foreground">當前候選(全域性)</div>
          <div className="text-[18px] font-bold mt-1">{globalCoverage?.total_signals ?? '--'}</div>
          <div className="text-[10px] text-muted-foreground mt-1">
            可執行: {globalCoverage?.active_signals ?? '--'}，觀察: {(globalCoverage?.total_signals != null && globalCoverage?.active_signals != null) ? Math.max(0, globalCoverage.total_signals - globalCoverage.active_signals) : '--'}
          </div>
        </div>
        <div className="card p-3">
          <div className="text-[11px] text-muted-foreground">市場池佔比</div>
          ...
          <div className="text-[10px] text-muted-foreground mt-1">
            市場池: {globalCoverage?.market_scan_signals ?? '--'}，關注池: {globalCoverage?.watchlist_signals ?? '--'}，融合: {globalCoverage?.mixed_signals ?? '--'}
          </div>
        </div>
        <div className="card p-3">
          <div className="text-[11px] text-muted-foreground">本次篩選結果</div>
          <div className="text-[18px] font-bold mt-1">{filteredSummary.total}</div>
          <div className="text-[10px] text-muted-foreground mt-1">
            未持倉: {filteredSummary.unheld}，市場池: {filteredSummary.marketPool}
          </div>
        </div>
        <div className="card p-3">
          <div className="text-[11px] text-muted-foreground">3日勝率(自動評估)</div>
          <div className="text-[18px] font-bold mt-1">{outcome3d ? `${outcome3d.win_rate.toFixed(1)}%` : '--'}</div>
          <div className="text-[10px] text-muted-foreground mt-1">
            自動樣本: {outcome3d ? `${outcome3d.total}` : '--'}
          </div>
        </div>
      </div>
      ...
      {(factorStats || constraintStats) && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
          <div className="card p-3">
            ...
            <div className="text-[10px] text-muted-foreground mt-1">樣本 {factorStats?.sample_size ?? '--'}</div>
          </div>
          <div className="card p-3">
            ...
            <div className="text-[10px] text-muted-foreground mt-1">
              擁擠懲罰 {factorStats ? factorStats.avg_crowd_penalty.toFixed(1) : '--'}
            </div>
          </div>
          <div className="card p-3">
            <div className="text-[11px] text-muted-foreground">平均質量/風險</div>
            ...
            <div className="text-[10px] text-muted-foreground mt-1">質量分越高越好</div>
          </div>
          <div className="card p-3">
            <div className="text-[11px] text-muted-foreground">組合約束降級</div>
            <div className="text-[18px] font-bold mt-1">{constraintStats?.constrained_top20 ?? 0}</div>
            <div className="text-[10px] text-muted-foreground mt-1">Top20 被風控降級數量</div>
          </div>
        </div>
      )}
      ...
      {(regimeSummary.length > 0 || riskSummary.length > 0) && (
        <div className="card p-3 mb-4">
          <div className="text-[11px] text-muted-foreground mb-2">市場狀態與組合風險</div>
          <div className="flex flex-wrap gap-2">
            ...
            {riskSummary.map((r) => (
              <span key={`risk-${r.market}`} className="text-[11px] px-2.5 py-1 rounded bg-accent/70 text-muted-foreground border border-border/60">
                {marketLabel(r.market)}風險: {r.riskLevel} · 集中度{(r.concentration * 100).toFixed(0)}% · 高風險佔比{(r.highRiskRatio * 100).toFixed(0)}%
              </span>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-8 gap-2">
          ...
          <Select value={source} onValueChange={(v) => setSource(v as SourceFilter)}>
            ...
            <SelectContent>
              ...
              <SelectItem value="market_scan">市場池</SelectItem>
              <SelectItem value="mixed">融合池</SelectItem>
              <SelectItem value="watchlist">關注池</SelectItem>
            </SelectContent>
          </Select>
          <Select value={holding} onValueChange={(v) => setHolding(v as HoldingFilter)}>
            <SelectTrigger className="h-8 text-[12px]"><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="all">全部持倉狀態</SelectItem>
              <SelectItem value="unheld">僅未持倉</SelectItem>
              <SelectItem value="held">僅持倉中</SelectItem>
            </SelectContent>
          </Select>
          ...
          <Select value={risk} onValueChange={(v) => setRisk(v as RiskFilter)}>
            <SelectTrigger className="h-8 text-[12px]"><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="all">全部風險等級</SelectItem>
          ...
          <Select value={minScore} onValueChange={setMinScore}>
            <SelectTrigger className="h-8 text-[12px]"><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="90">評分90+</SelectItem>
              <SelectItem value="80">評分80+</SelectItem>
              <SelectItem value="70">評分70+</SelectItem>
              <SelectItem value="60">評分60+</SelectItem>
              <SelectItem value="50">評分50+</SelectItem>
              <SelectItem value="0">評分不過濾</SelectItem>
            </SelectContent>
          </Select>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {groupedItems.map((group) => {
          ...
          if (group.hasMarketScan) sourceFlags.push('市場候選')
          if (inWatchlist) sourceFlags.push('已關注標的')
          if (sourceFlags.length <= 0) sourceFlags.push('關注池')
          const sourcePoolLabel = group.hasMarketScan
            ? (group.members.some((x) => x.source_pool === 'mixed') ? '市場+關注' : '市場池')
            : (item.source_pool_label || '關注池')
          return (
            <div key={stateKey} className={`card p-4 transition-colors ${toneClass(item)}`}>
              <button className="w-full text-left" onClick={() => openInsight(item)}>
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="text-right">
                    ...
                    <div className={`text-[12px] font-mono mt-1 ${Number(item.rank_score || item.score || 0) >= 80 ? 'text-primary' : 'text-muted-foreground'}`}>
                      評分 {Math.round(item.rank_score || item.score || 0)}
                    </div>
                    {item.ai_score != null && (
                ...
                <div className="mt-2 grid grid-cols-2 gap-2 text-[11px] text-muted-foreground">
                  <div>入場: {formatEntryDisplay(item.action, entryLow, entryHigh)}</div>
                  <div>停損: {formatPlanPrice(stopLoss)}</div>
                  <div>目標: {formatPlanPrice(targetPrice)}</div>
                  <div>失效: {item.invalidation || '--'}</div>
                  <div>
                    策略: {strategyHead}
                    {strategyTailCount > 0 ? ` +${strategyTailCount}` : ''}
                  </div>
                  <div>來源池: {sourcePoolLabel}</div>
                  <div>
                    來源Agent: {sourceAgentHead}
                    {sourceAgentTailCount > 0 ? ` +${sourceAgentTailCount}` : ''}
                  </div>
                  <div>風險: {item.risk_level_label || item.risk_level || '--'}</div>
                  <div>市場狀態: {marketRegime.regime_label || marketRegime.regime || '--'}</div>
                  <div>持倉: {item.is_holding_snapshot ? '持倉中' : '未持倉'}</div>
                  <div>市場: {marketLabel(item.stock_market)}</div>
                </div>
                <div className="mt-2 grid grid-cols-2 gap-2 text-[10px] text-muted-foreground">
                  <div>Alpha: {formatMetric(breakdown.alpha_score)}</div>
                  <div>催化: {formatMetric(breakdown.catalyst_score)}</div>
                  <div>質量: {formatMetric(breakdown.quality_score)}</div>
                  <div>風險懲罰: {formatMetric(breakdown.risk_penalty)}</div>
                  <div>相對強弱: {crossFeature.relative_strength_pct != null ? `${Number(crossFeature.relative_strength_pct).toFixed(0)}分位` : '--'}</div>
                  <div>事件催化: {eventScore != null ? eventScore.toFixed(1) : '--'}{eventCount > 0 ? `（${eventCount}條）` : '（無命中）'}</div>
                </div>
                ...
                {item.constrained && (
                  <div className="mt-2 text-[10px] text-amber-400">
                    組合約束: {(item.constraint_reasons || []).join('；') || '已自動降級'}
                  </div>
                )}
              </button>
              ...
              <div className="mt-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-[10px] text-muted-foreground">評估: 自動後驗</div>
                </div>
              </div>
            </div>
          )
      ...
      {!loading && groupedItems.length === 0 && (
        <div className="card p-8 text-center text-[12px] text-muted-foreground mt-4">暫無滿足條件的機會</div>
      )}
