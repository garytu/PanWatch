...
const config = {
  ...
  models: [
    { id: 6, name: 'DeepSeek V4 Flash', model: 'deepseek-ai/DeepSeek-V4-Flash', service_name: '矽基流動' },
  ],
}
...
describe('AssistantConfigPanel', () => {
  ...
  it('loads and saves the context compression model and budget', async () => {
    ...
    expect((await screen.findByLabelText('上下文壓縮模型') as HTMLSelectElement).value).toBe('6')
    fireEvent.change(screen.getByLabelText('最大上下文 Token'), { target: { value: '16000' } })
    fireEvent.click(screen.getByRole('button', { name: '儲存上下文配置' }))
