...
def test_sina_us_quote(monkeypatch):
    # US: gb_ 逗號欄位,idx 0 name,1 price,2 change%,5 open,6 high,7 low,10 vol,14 pe,26 prev_close
    parts = ["0"] * 30
    parts[0] = "蘋果"; parts[1] = "150.5"; parts[2] = "1.2"; parts[5] = "149.0"
    ...
    q = out[0]
    assert q.symbol == "AAPL" and q.market == "US" and q.name == "蘋果"
...
def test_sina_hk_quote(monkeypatch):
    # HK: rt_hk 逗號欄位,idx 1 name,2 open,3 prev_close,4 high,5 low,6 price,7 change,8 change%,11 amount,12 vol
    parts = ["0"] * 15
    parts[1] = "騰訊控股"; parts[2] = "300.0"; parts[3] = "298.0"; parts[4] = "305.0"
    ...
    q = out[0]
    assert q.symbol == "00700" and q.market == "HK" and q.name == "騰訊控股"
    assert q.current_price == 302.0 and q.prev_close == 298.0 and q.change_pct == 1.34


def test_sina_cn_unsupported():
    # CN 不支援 → supports_markets 攔截(vendor 只 US/HK);此處直接調不傳 CN
