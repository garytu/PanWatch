...
def save_stock_context_snapshot(
    *,
    ...
    try:
        ...
        logger.warning(f"儲存 stock context snapshot 失敗: {e}")
        db.rollback()
        return False
    finally:
        db.close()
...
def save_news_topic_snapshot(
    *,
    ...
    try:
        ...
        logger.warning(f"儲存 news topic snapshot 失敗: {e}")
        db.rollback()
        return False
    finally:
        db.close()
...
def save_agent_context_run(
    *,
    ...
    try:
        ...
        logger.warning(f"儲存 agent context run 失敗: {e}")
        db.rollback()
        return False
    finally:
        db.close()
    ...
    try:
        ...
        db.add(
            AgentPredictionOutcome(
                ...
                action_label=action_label or "觀望",
        ...
        logger.warning(f"儲存 prediction outcome 失敗: {e}")
        db.rollback()
        return False
    finally:
        db.close()
...
def mark_agent_prediction_outcome(
    *,
    ...
    try:
        ...
        logger.warning(f"更新 prediction outcome 失敗: {e}")
        db.rollback()
        return False
    finally:
        db.close()
...
def cleanup_context_data(
    *,
    ...
    try:
        ...
        logger.warning(f"清理 context 資料失敗: {e}")
        db.rollback()
        return deleted
    finally:
        db.close()
