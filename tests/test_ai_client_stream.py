"""AIClient.chat_stream 流式通道單測（全部 mock，不發真實請求）。"""
...
def _make_client(chunks):
    """構造一個 completions.create 返回固定 chunk 序列的 AIClient。"""
...
def test_stream_tokens():
    """純文本流：逐 token 產出，最後產出完整 message"""
    client = _make_client([
        _chunk(content="你"),
        _chunk(content="好"),
        _chunk(content="！"),
...
def test_stream_tool_calls_assembled():
    """工具呼叫流：分片的 arguments 按 index 聚合成完整 tool_calls"""
    client = _make_client([
...
def test_stream_usage_and_empty_choices():
    """末尾只含 usage 的空 choices chunk 不報錯，且累計 token 用量"""
