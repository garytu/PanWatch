"""評分整合(M3):_compute_factor_breakdown 按每因子權重加權 + 零迴歸。"""
...
def _candidate(**kw):
    """構造一個記憶體 EntryCandidate(不入庫),score=80 → 原始 alpha_score=13.5。"""
    defaults = dict(
...
def test_factor_weights_none_equals_empty_zero_regression():
    """factor_weights = None / {} / 全 1.0 三者輸出完全一致(零迴歸)。"""
    row = _candidate()
    a = _bd(row, None)
    b = _bd(row, {})
    c = _bd(row, {
